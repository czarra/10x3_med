...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "Główny cel aplikacji"
      decision: "Analiza danych i korygowanie przeliczników w celu unikania wahań cukru"
    - topic: "Główna persona"
      decision: "Osoba z cukrzycą typu 1 samodzielnie kontrolująca chorobę"
    - topic: "Kluczowe wyróżniki (Insights)"
      decision: "Dynamiczne wyliczanie przelicznika Insulina/WW, wpływ wysiłku na dawkę, oraz maksymalnie uproszczone wprowadzanie danych"
    - topic: "Dostęp i logowanie"
      decision: "Rejestracja i logowanie (e-mail + hasło) w celu zapisu danych w chmurze (OAuth w v2)"
    - topic: "Model ról (Access Control)"
      decision: "Dwurzędowy model odłożony na v2; w v1 skupiamy się na jednym profilu pacjenta z opcją eksportu raportów"
    - topic: "Zakres MVP i budżet czasowy"
      decision: "Zawężony zakres (bez kont diabetologa w v1, uproszczony wysiłek fizyczny), planowany czas: 3 tygodnie pracy po godzinach"
    - topic: "Główne kryteria sukcesu"
      decision: "Sugestia przelicznika po 3 wpisach posiłkowych + ostrzeżenia o hipoglikemii przy wysiłku"
    - topic: "Algorytm i reguły biznesowe"
      decision: "Analiza przelicznika posiłkowego Insulina/WW oraz analiza stabilności glikemii w celu sugestii korekty insuliny bazowej (dobowej)"
    - topic: "Wymagania niefunkcjonalne (NFR)"
      decision: "Szyfrowanie danych, stały disclaimer medyczny, brak restrykcyjnego limitu <200ms na kalkulację"
    - topic: "Typ produktu"
      decision: "Aplikacja internetowa (web-app)"
    - topic: "Skala docelowa"
      decision: "Mała skala (użytek własny/znajomi)"
    - topic: "Ograniczenia MVP (Non-Goals)"
      decision: "Brak CGM, brak kont lekarzy, brak automatycznego wyliczania dla wielu marek insuliny, brak trybu offline"
    - topic: "Edycja wpisów"
      decision: "Zezwolenie na edycję/usuwanie ostatnich wpisów w celu korekty błędów bez wpływu na starszą historię"
  frs_drafted: 14
...
- **Bezpieczeństwo danych**: Dane glikemii i historii leczenia jako dane wrażliwe są szyfrowane w bazie danych i niedostępne dla podmiotów trzecich bez wyraźnej zgody pacjenta.
...
Rekomendacje te są generowane w tle przez silnik analityczny na podstawie zgromadzonych w bazie wpisów użytkownika. Wyświetlają się na pulpicie głównym w postaci sugestii, które pacjent musi ręcznie zatwierdzić, co powoduje zaktualizowanie jego parametrów w profilu.

## Non-Functional Requirements

- **Szyfrowanie i bezpieczeństwo danych**: Jako że dane glikemii i dawek insuliny są danymi medycznymi (wrażliwymi), są one szyfrowane zarówno w spoczynku (w bazie danych), jak i podczas przesyłania. Dostęp do nich ma wyłącznie uwierzytelniony użytkownik.
...
- **Wizualna informacja o kalkulacji**: W przypadku gdy przetwarzanie danych i kalkulacja trendów trwa dłużej niż 500 ms, interfejs w czytelny sposób (np. loader / szkielet interfejsu) sygnalizuje użytkownikowi trwające obliczenia, zapobiegając wrażeniu zablokowania aplikacji.
