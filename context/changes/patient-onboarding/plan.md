...
- `symfony/maker-bundle`'s `MakeAuthenticator` (`vendor/symfony/maker-bundle/src/Maker/MakeAuthenticator.php`) generates a custom `AbstractLoginFormAuthenticator` subclass registered under `custom_authenticators` — this is the current Symfony 7 idiom, not the older plain `form_login:` config key.
...
**File**: `src/Security/LoginFormAuthenticator.php` (generated)

**Intent**: Generate via `bin/console make:authenticator`, choosing the "Login form authenticator" option, firewall `main`, user class `App\Entity\User`, username field `email`. Customize the success redirect.

**Contract**: `extends AbstractLoginFormAuthenticator`. `onAuthenticationSuccess`: use `TargetPathTrait::getTargetPath()` to honor a page the user was bounced from (e.g. by `access_control`), falling back to redirecting to the `patient_profile` route (not the maker's default `/`) when there's no target path.

#### 2. Security configuration

**File**: `config/packages/security.yaml`

**Intent**: Wire the generated authenticator and enable logout on the `main` firewall (the route loader in `config/routes/security.yaml` is already waiting on this). This is also what unblocks Phase 4's `Security::login()` auto-login call — see that phase's Registration controller contract.

**Contract**: `main` firewall gains `custom_authenticators: [App\Security\LoginFormAuthenticator]` and a `logout:` block (path `/logout`, generated by `make:authenticator`'s interactive prompts — accept its defaults). Add `/login` as a public `access_control` entry only if the maker doesn't already do so (verify `access_control` still has the Phase 2 entry unchanged); `/register` isn't public yet since Phase 4 hasn't created that route.

#### 3. Login controller + template

**Files**: `src/Controller/SecurityController.php` (generated), `templates/security/login.html.twig` (generated)

**Intent**: Standard maker-generated login form (route `/login`, name `app_login`) plus a bare logout action (`/logout`, name `app_logout`) that Symfony's security system intercepts before the controller body runs.

**Contract**: Accept the maker's generated shape; adjust template copy to match the app's plain-Polish-Twig convention.
...
**Contract**: `#[Route('/register', name: 'app_register', methods: ['GET', 'POST'])]`. On valid submit: hash `plainPassword` via `UserPasswordHasherInterface`, set on the new `User`, persist + flush, call `Security::login($user, LoginFormAuthenticator::class, 'main')` (from `Symfony\Bundle\SecurityBundle\Security`, explicit authenticator + firewall name — this is what `make:registration-form` itself generates once it detects the single `custom_authenticators` entry Phase 3 registers, per `vendor/symfony/maker-bundle/templates/registration/RegistrationController.tpl.php:46`; keep the maker's generated form as-is rather than reducing it to the zero-argument `Security::login($user)` overload — the zero-arg lookup only works while the firewall has exactly one authenticator and starts throwing `LogicException('Too many authenticators...')` the moment a second one is ever added, e.g. remember-me; this is also why registration is sequenced after login/logout, since Phase 3 must register that one authenticator first), redirect to `patient_onboarding`. On duplicate e-mail (Doctrine unique constraint violation, or a pre-flush existence check via `UserRepository`), re-show the form with a field-level error rather than a 500.
...
- [ ] 3.2 Code style passes: `vendor/bin/php-cs-fixer fix --dry-run --diff`
- [ ] 3.3 Functional tests pass: `SecurityControllerTest.php`

#### Manual

- [ ] 3.4 Directly-seeded user logs in/out via browser, lands correctly, `/profil` redirects to `/login` after logout
